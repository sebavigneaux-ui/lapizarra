...
export default function PDFDownloadButton({ data }: { data: CotizacionData }) {
  return (
    <PDFDownloadLink
      ...
      className="flex items-center gap-2 px-6 py-3 rounded-full font-black text-sm border border-white/25 text-white/60 hover:text-white hover:border-white/50 transition-all duration-200"
    >
      {({ loading }) =>
        loading ? (
          <>
